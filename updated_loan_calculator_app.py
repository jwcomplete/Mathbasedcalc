...
with col1:
    ...
    purchase_price = float(st.number_input("💰 Price ($)", value=50000.0, min_value=50000.0, max_value=999999999.0, step=5000.0, format="%.0f"))

with col2:
    loan_term = float(st.number_input("📆 Term (Years)", value=30.0, min_value=5.0, max_value=30.0, step=5.0, format="%.0f"))
    interest_rate = float(st.number_input("📊 Interest (%)", value=5.625, min_value=1.0, max_value=10.0, step=0.001, format="%.3f"))

with col3:
    property_tax = float(st.number_input("🏡 Tax ($)", value=0.0, min_value=0.0, max_value=50000.0, step=0.01, format="%.2f"))
    home_insurance = float(st.number_input("🔒 Insurance ($)", value=0.0, min_value=0.0, max_value=20000.0, step=0.01, format="%.2f"))
    flood_insurance = float(st.number_input("🌊 Flood Ins. ($)", value=0.0, min_value=0.0, max_value=20000.0, step=0.01, format="%.2f"))
